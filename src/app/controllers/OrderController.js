  ...
  try {
    ...
    if (
      ...
      order = new Order({
        ...
        total:
          discountDB.category == 'shippingFee'
            ? cart.subtotal
            : cart.subtotal + 20000,
        discount:
          discountDB.category == 'rate'
            ...
            : discountDB.category == 'money'
            ? discountDB.minusPrice.toLocaleString('vi', {
                style: 'currency',
                currency: 'VND',
              })
            : 'Free ship',
  ...
  try {
    ...
    if (req.body.user) {
      ...
      customer.notification = [
        ...customer.notification,
        {
          message: `Đơn hàng #${order._id
            .toString()
            .slice(16, 24)} đã được bạn đặt tại quán`,
          time: new Date(),
...
const createOrderByGuest = async (req, res) => {
  try {
    ...
    const allProduct = [
      ...(await Product.find({ category: 'Cà phê' })
        .sort({ _id: -1 })
        .limit(3)),
      ...(await Product.find({ category: 'Trà sữa' })
        .sort({ _id: -1 })
        .limit(3)),
      ...(await Product.find({ category: 'Trà trái cây' })
        .sort({ _id: -1 })
        .limit(3)),
      ...(await Product.find({ category: 'Đá xay' })
        .sort({ _id: -1 })
        .limit(3)),
      ...(await Product.find({ category: 'Matcha' })
        .sort({ _id: -1 })
        .limit(3)),
        ...
        ...customer.notification,
        {
          message:
            order.status == 'Shipper đang giao'
              ? `Đơn hàng #${order._id
                  .toString()
                  .slice(16, 24)} của bạn đang được giao`
              : `Đơn hàng #${order._id
                  .toString()
                  .slice(16, 24)} của bạn đã giao thành công`,
          time: new Date(),
